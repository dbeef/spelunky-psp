...
void Cube::write()
{
...

    {
        ...
        glm::mat4x4 translation_matrix = glm::translate(glm::identity<glm::mat4x4>(),
                                                      glm::vec3{_base_quad.get_translation_x(),
                                                                   _base_quad.get_translation_y() - 0.0f,
    ...
    {
        Quad left_side = _base_quad;
        left_side.set_translation(0, 0, 0);
        left_side.write();

        glm::mat4x4 translation_matrix = glm::translate(glm::identity<glm::mat4x4>(),
                                                      glm::vec3{_base_quad.get_translation_x(),
                                                                   _base_quad.get_translation_y(),
                                                                   _base_quad.get_translation_z() + 1.0f});

        for (int i = 0; i < Quad::get_vertices_count(); i++)
        {
            Vertex v_original = *(left_side.get_vertices_base() + i);
            glm::vec4 v_rotated = {v_original.x, v_original.y, v_original.z, 1.0f};
        ...
        for (int i = 0; i < Quad::get_indices_count(); i++)
        {
            _indices.push_back(*(left_side.get_indices() + i) + (num_sides * 4));
        }
    ...
    {
        Quad right_side = _base_quad;
        right_side.set_translation(0, 0, 0);
        right_side.write();

        glm::mat4x4 translation_matrix = glm::translate(glm::identity<glm::mat4x4>(),
                                                      glm::vec3{_base_quad.get_translation_x() + 1.0f,
                                                                   _base_quad.get_translation_y(),
                                                                   _base_quad.get_translation_z() + 1.0f});

        for (int i = 0; i < Quad::get_vertices_count(); i++)
        {
            Vertex v_original = *(right_side.get_vertices_base() + i);
            glm::vec4 v_rotated = {v_original.x, v_original.y, v_original.z, 1.0f};
        ...
        for (int i = 0; i < Quad::get_indices_count(); i++)
        {
            _indices.push_back(*(right_side.get_indices() + i) + (num_sides * 4));
        }
