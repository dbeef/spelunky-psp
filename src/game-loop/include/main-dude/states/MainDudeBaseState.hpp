#pragma once

#include <cstdint>

class Input;
class MainDude;
class World;

class MainDudeBaseState
{
public:
    ~MainDudeBaseState() = default;

    virtual MainDudeBaseState* update(MainDude&, World* world, uint32_t delta_time_ms) = 0;
    virtual MainDudeBaseState* handle_input(MainDude&, World* world, const Input& input) = 0;
    virtual void enter(MainDude&) = 0;
    virtual void exit(MainDude&) {}; // Default definition provided as most states don't utilize the exit function.
};
